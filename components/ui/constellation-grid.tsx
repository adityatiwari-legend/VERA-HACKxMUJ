...
export default function ConstellationGrid() {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // Draw VERA text directly into canvas with dynamic scroll fade (fades completely by scrollProgress ~0.14)
            const scrollP = typeof window !== 'undefined' ? ((window as any).__veraScrollProgress ?? 0) : 0;
            const textAlpha = Math.max(0, 1.0 - scrollP * 7.0);
            ...
            if (textAlpha > 0.005) {
                ...
                ctx.font = '900 24vw system-ui, -apple-system, BlinkMacSystemFont, sans-serif';
                ctx.letterSpacing = '0.06em';
                ctx.fillStyle = isDarkMode ? 'rgba(255,255,255,1)' : 'rgba(15,23,42,1)';
                ctx.shadowColor = `rgba(0,0,0,${0.8 * textAlpha})`;
                ctx.shadowBlur = 35;
                ctx.shadowOffsetY = 10;
                ctx.fillText('V E R A', width / 2, height / 2);
                ctx.restore();
            }
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                if (dist < 90) {
                    const rawPulse = ((Math.abs(n.pulse) * 20) % 30) + 4;
                    const pulseRing = Math.max(0.5, rawPulse);
                    const ringAlpha = Math.max(0, (1 - pulseRing / 34) * 0.4);
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden select-none bg-slate-950">
            <canvas id="constellation-bg-canvas" ref={canvasRef} className="absolute inset-0 block cursor-crosshair" />
